...
$programmes=[];
$result_progs=mysqli_query($conn,"SELECT t_id,prog_name,prog_code,department_id FROM programme ORDER BY prog_name");
while($row=mysqli_fetch_assoc($result_progs))$programmes[]=$row;
$levels=[];
...
// Programme must belong to the chosen department.
if(empty($errors)){
...
if(mysqli_stmt_get_result($stmt_prog)->num_rows==0)$errors[]='The selected programme does not belong to the selected department.';
...
<option value="<?php echo $prog['t_id'];?>" data-department="<?php echo $prog['department_id'];?>" <?php echo(isset($_POST['programme_id'])&&$_POST['programme_id']==$prog['t_id'])?'selected':'';?>>
<?php echo htmlspecialchars($prog['prog_name']);?>
</option>
<?php endforeach;?>
</select>
<small style="color:#666">Must belong to the selected department</small>
</div>
<div class="form-group">
