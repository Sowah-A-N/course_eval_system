...
if (isset($_GET['action']) && $_GET['action'] === 'template') {
    ...
    fputcsv($out, ['course_code', 'course_name', 'level_id', 'semester_id', 'credit_hours']);
    fputcsv($out, ['CS101', 'Introduction to Computing', '1', '1', '3']);
    fclose($out);
    exit();
}
 ...
 * Helpers
 * --------------------------------------------------------------------- */
function get_levels_map_c(mysqli $conn): array {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'confirm') {
    ...
    try {
        $stmt_ins = mysqli_prepare($conn,
            "INSERT INTO courses (course_code, name, department_id, level_id, semester_id, credit_hours, created_at)
             VALUES (?, ?, ?, ?, ?, ?, NOW())");
        ...
        foreach ($preview as $row) {
            mysqli_stmt_bind_param($stmt_ins, "ssiiii",
                ...
                $department_id, $row['level_id'], $row['semester_id'], $row['credit_hours']);
            if (!mysqli_stmt_execute($stmt_ins)) {
                throw new RuntimeException("DB insert failed for {$row['course_code']}: " . mysqli_stmt_error($stmt_ins));
            }
            $inserted++;
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') !== 'confirm') {
    ...
    if (empty($parse_errors)) {
        $levels    = get_levels_map_c($conn);
        $semesters = get_semesters_map($conn);
        $col = import_header_map($rows[0]);
        $missing = array_diff(['course_code', 'course_name', 'level_id', 'semester_id', 'credit_hours'], array_keys($col));
        if (!empty($missing)) {
            ...
            for ($i = 1; $i < $total; $i++) {
                ...
                $course_code     = strtoupper(import_cell($rows[$i], $col, 'course_code'));
                $course_name     = import_cell($rows[$i], $col, 'course_name');
                $level_id_raw    = import_cell($rows[$i], $col, 'level_id');
                $semester_id_raw = import_cell($rows[$i], $col, 'semester_id');
                $credit_hours_raw= import_cell($rows[$i], $col, 'credit_hours');
                ...
                if ($semester_id === false || $semester_id <= 0) {
                    $row_errors[] = 'semester_id must be a positive integer';
                } elseif (!isset($semesters[$semester_id])) {
                    $row_errors[] = "semester_id $semester_id does not exist";
                }

                $credit_hours = filter_var($credit_hours_raw, FILTER_VALIDATE_INT);
                if ($credit_hours === false || $credit_hours < 1 || $credit_hours > 6) {
                    $row_errors[] = 'credit_hours must be between 1 and 6';
                }

                $is_valid = empty($row_errors);
                if ($is_valid) { $valid_count++; $seen_codes[$course_code] = true; }
                else $error_count++;
                ...
                $preview_rows[] = [
                    ...
                    'semester_id'  => $semester_id !== false ? $semester_id : 0,
                    'semester_name'=> ($semester_id && isset($semesters[$semester_id])) ? $semesters[$semester_id] : $semester_id_raw,
                    'credit_hours' => $credit_hours !== false ? $credit_hours : $credit_hours_raw,
                    'valid'        => $is_valid,
                    'errors'       => $row_errors,
                ];
            }
...
<div class="page-header">
    <h1>Bulk Course Import</h1>
    <p>Upload a CSV or Excel file to add multiple courses at once</p>
</div>
...
<div class="alert alert-<?php echo $_SESSION['flash_type'] === 'error' ? 'error' : 'info'; ?>">
    ...
    <br><a href="list.php" style="color:inherit;font-weight:600">View All Courses →</a>
    <?php endif; ?>
</div>
...
<div class="card">
    ...
    <div class="table-wrap">
        <table>
            <thead><tr><th>Row</th><th>Code</th><th>Course Name</th><th>Level</th><th>Semester</th><th>Credits</th><th>Status</th></tr></thead>
            <tbody>
            <?php foreach ($preview_rows as $pr): ?>
            <tr class="<?php echo $pr['valid'] ? 'row-valid' : 'row-error'; ?>">
                <td><?php echo (int)$pr['row']; ?></td>
                <td><?php echo htmlspecialchars($pr['course_code']); ?></td>
                <td><?php echo htmlspecialchars($pr['course_name']); ?></td>
                <td><?php echo htmlspecialchars($pr['level_name']); ?></td>
                <td><?php echo htmlspecialchars($pr['semester_name']); ?></td>
                <td><?php echo htmlspecialchars((string)$pr['credit_hours']); ?></td>
                <td>
                    <?php if ($pr['valid']): ?>
    ...
    <form method="POST">
        ...
        <button type="submit" class="btn btn-success">Confirm Import (<?php echo $valid_count; ?> course<?php echo $valid_count !== 1 ? 's' : ''; ?>)</button>
...
<div class="card">
    ...
    <p class="card-subtitle">Import multiple courses by uploading a CSV (.csv) or Excel (.xlsx) file.</p>

    <div class="template-hint">
        <strong>Required columns:</strong>
        <code>course_code</code>, <code>course_name</code>, <code>level_id</code>, <code>semester_id</code>, <code>credit_hours</code>
    ...
    <div style="display:flex;gap:32px;flex-wrap:wrap;margin-bottom:18px">
        <details>
            ...
            <table style="margin-top:8px;width:auto;min-width:220px">
                <thead><tr><th>Level ID</th><th>Level Name</th></tr></thead>
                <tbody>
                <?php foreach (get_levels_map_c($conn) as $id => $name): ?>
        ...
        <details>
            <summary style="cursor:pointer;font-size:13px;font-weight:600;color:#667eea">View Semester ID Reference</summary>
            <table style="margin-top:8px;width:auto;min-width:220px">
                <thead><tr><th>Semester ID</th><th>Semester Name</th></tr></thead>
                <tbody>
                <?php foreach (get_semesters_map($conn) as $id => $name): ?>
                <tr><td><?php echo $id; ?></td><td><?php echo htmlspecialchars($name); ?></td></tr>
                <?php endforeach; ?>
                </tbody>
            </table>
