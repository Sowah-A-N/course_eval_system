...
-- ================================================================
-- academic_year
-- Note: MariaDB 5.5 computed column syntax is AS (...) PERSISTENT
-- ================================================================
...
CREATE TABLE `classes` (
  ...
  `level_id`           INT,
  `created_at`         TIMESTAMP   NOT NULL DEFAULT CURRENT_TIMESTAMP,
  UNIQUE KEY `uq_class_code_dept` (`class_code`,`department_id`),
...
-- advisor_user_id back-filled after user_details is populated
...
-- All assigned to the active period: semester_id=6 (2025/2026 Second)
...
-- bcrypt hash        : $2y$12$olRKMrkG2EVWcT2ReasNpOoAeS6j92zDcekHK.AUEINGjq/Sfom6q
...
-- ── Back-fill department HOD references ──────────────────────
UPDATE `department` SET `hod_id`=3 WHERE `t_id`=1;
UPDATE `department` SET `hod_id`=4 WHERE `t_id`=2;

-- ── Back-fill class advisors ──────────────────────────────────
UPDATE `classes` SET `advisor_user_id`=7 WHERE `t_id`=1;  -- BIT28 -> Kwame
UPDATE `classes` SET `advisor_user_id`=8 WHERE `t_id`=2;  -- BIT27 -> Ama
UPDATE `classes` SET `advisor_user_id`=7 WHERE `t_id`=3;  -- BIT26 -> Kwame
UPDATE `classes` SET `advisor_user_id`=8 WHERE `t_id`=4;  -- BCS28 -> Ama
UPDATE `classes` SET `advisor_user_id`=9 WHERE `t_id`=5;  -- EEE28 -> Kofi
UPDATE `classes` SET `advisor_user_id`=9 WHERE `t_id`=6;  -- EEE27 -> Kofi
...
VALUES
( 1,7,3,6,3),  -- BINT101 -> Kwame   (assigned by HOD ICT)
...
( 9,9,3,6,4),  -- BEEE101 -> Kofi    (assigned by HOD EEE)
(10,9,3,6,4);  -- BEEE102 -> Kofi

-- ================================================================
-- evaluation_questions  (20 questions across all 9 categories)
-- ================================================================
...
-- ================================================================
-- evaluation_tokens
-- One row per (student, course) for the active period
-- is_used=1 rows have student_user_id nulled afterwards (anonymity)
-- ================================================================
DROP TABLE IF EXISTS `evaluation_tokens`;
CREATE TABLE `evaluation_tokens` (
  `token_id`         INT         AUTO_INCREMENT PRIMARY KEY,
  `student_user_id`  INT         NULL DEFAULT NULL,
  `course_id`        INT,
  `academic_year_id` INT,
  `semester_id`      INT,
...
VALUES
-- BIT28 x BINT101 (course 1) : students 10-17 — 10,17 pending
...
-- BIT28 x BINT102 (course 2) : 13 pending
...
-- BIT28 x BINT103 (course 3) : 15 pending
...
-- BIT27 x BINT201 (course 4) : 21 pending
...
-- BIT27 x BINT202 (course 5) : 20 pending
...
-- BIT26 x BINT301 (course 6) : 26 pending
...
-- BIT26 x BINT302 (course 7) : 25 pending
...
-- EEE28 x BEEE101 (course 9) : 30 pending
...
-- EEE28 x BEEE102 (course 10) : 28 pending
...
-- Score key:
--   BINT101 (Kwame) evals  1- 6 : high   avg ~4.5
--   BINT102 (Ama)   evals  7-13 : good   avg ~4.3
--   BINT103 (Kwame) evals 14-20 : high   avg ~4.4
--   BINT201 (Ama)   evals 21-24 : mid-hi avg ~4.1
--   BINT202 (Kwame) evals 25-28 : mid    avg ~3.5
--   BINT301 (Kwame) evals 29-31 : high   avg ~4.6
--   BINT302 (Ama)   evals 32-34 : varied avg ~4.0
--   BEEE101 (Kofi)  evals 35-37 : mid    avg ~3.4
--   BEEE102 (Kofi)  evals 38-40 : low    avg ~2.9
-- Facility/service questions (12-20) : 3-4 throughout
-- ================================================================
...
-- BEEE102 evals 38-40 (Kofi — low-mid)
INSERT INTO `responses` (`evaluation_id`,`question_id`,`response_value`) VALUES
...
-- ================================================================
-- TEST ACCOUNT SUMMARY  (all passwords: password)
-- ----------------------------------------------------------------
-- Role       Username      Dept   Notes
-- admin      admin         -      Full system access
-- quality    quality1      ICT    Quality reports
-- hod        hod_ict       ICT    Manages ICT lecturers & courses
-- hod        hod_eee       EEE    Manages EEE lecturers & courses
...
-- ----------------------------------------------------------------
-- DATA COVERAGE FOR REPORTS
--   Courses with responses : BINT101-103, BINT201-202,
--                             BINT301-302, BEEE101, BEEE102
--   Token completion rates : 75-88% (some intentionally pending)
--   Score range            : BEEE102 avg ~2.9 -> BINT101/301 avg ~4.5
--   ICT students : 24  |  EEE students : 7
--   Levels with data : 100, 200, 300 (400 course exists, no tokens yet)
-- ================================================================
